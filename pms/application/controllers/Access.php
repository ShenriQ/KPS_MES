...
class Access extends Application_controller {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if(!isset($select_package)) {
					...
					try {
						...
						$this->db->simple_query("UPDATE companies SET created_by_id = ".$user->getId()." WHERE id = ".$company->getId()." ");
						$this->db->simple_query("UPDATE target_sources SET created_by_id = ".$user->getId()." WHERE id = ".$source_id." ");
	...
	public function login() {
		...
		if(logged_user() instanceof User) redirect('dashboard');
