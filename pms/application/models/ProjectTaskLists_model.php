...
class ProjectTaskLists_model extends Application_model {
	...
	public function getDatedByProject(Project $project, $include_private = false, $include_trashed = false, $limit = null, $offset = null) {
		...
		return $this->find(array('conditions' => array('is_private <= ? AND is_trashed <= ? AND project_id = ? AND (start_date <> \'2021/01/01\' AND due_date <> \'2021/01/01\')', $include_private, $include_trashed, $project->getId()), 
		'order' => 'sort_order ASC, id DESC',
		'limit' => $limit,
        'offset' => $offset
		));
